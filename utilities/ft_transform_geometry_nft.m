function [output] = ft_transform_geometry_nft(transform, input)

% FT_TRANSFORM_GEOMETRY_NFT applies a homogeneous coordinate transformation to
...
%
%   output = ft_transform_geometry(transform, input,'warp',nftwarp) *** JRI ***
%
% Directly adapted from ft_transform_geometry, but with added warping of
% geometry to a template brian (warping determined by NFT toolkit)
%
...
nftwarp = [];
if nargin > 2
  nftwarp = ft_getopt(varargin, 'warp');
end
...
if ~allowscaling
  ...
  if (abs(det(rotation))-1)>1e-6
    warning('only a rigid body transformation without rescaling is allowed'); % *** JRI *** downgraded, I do rescale
  end
end
...
%for warping of individual geometry to template--this may cover all there is?
wfields = {'pos','pnt'};
...
for k = 1:numel(fnames)
  if ~isempty(input.(fnames{k}))
    ...
    % *** do warping towards a template brain
    if ~isempty(nftwarp) && any(strcmp(fnames{k}, wfields))
      %fprintf('NFT warp of field %s\n',fnames{k})
      input.(fnames{k}) = warp_lm(input.(fnames{k}), nftwarp);
    end
    % ***
