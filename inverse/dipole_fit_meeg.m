function [dipout] = dipole_fit_meeg(dip, sens, headmodel, dat, varargin)
% DIPOLE_FIT performs an equivalent current dipole fit with a single
% or a small number of dipoles to explain a joint M/EEG scalp topography.
%
% Use as
%   [dipout] = dipole_fit_meeg(dip, sens, headmodel, dat, ...)
%
...
%   'metric'      = Error measure to be minimised [ rv | var | abs | rv_split* ]
%                     *rv_split takes of mean rv of different modalities as error measure
...
if ~iscell(sens) && ~iscell(vol)
  error('this function is for multimodal data; you should be using dipole_fit instead')
end

if ~isfield(sens{1},'selsens')
  error('sensor structs must have a selsens field indexing channels in full dataset')
end

% *** JRI ***
if strcmp(metric,'rv_split') && istrue(normalize)
  error('rv_split does not need normalization')
end
% *** JRI ***
...
if exitflag==0
  warning('Maximum number of iterations exceeded before reaching the minimum, please try with another initial guess.')
end
...
if ~isempty(weight)
  ...
  switch metric
    case 'rv' % relative residual variance (per modality, summed)
      err = sum(dif(:).^2) / sum(dat(:).^2);
    case 'rv_split' %(weighted) mean of rv within modality--in this case, no need to normalize lf or data!
      for iM = 1:length(sens)
        idx = sens{iM}.selsens;
        tmpdif = dif(idx,:); %not sure if multi column is possible here
        tmpdat = dat(idx,:);
        errm(iM) = sum(tmpdif(:).^2) / sum(tmpdat(:).^2);
      end
      err = mean(errm);
      err_all = sum(dif(:).^2) / sum(dat(:).^2);
      fprintf('e: %.3f (%.3f, %.3f) = %.3f \n',err_all, errm(1), errm(2), err);
    case 'var' % residual variance
      err = sum(dif(:).^2);
